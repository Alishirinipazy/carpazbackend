...
from app.api.v1.inquiries import _serialize as _serialize_inquiry, _with_relations as _with_inquiry_relations
from app.db.session import get_db
from app.models import City, Inquiry, Province, User, UserAddress
...
def my_inquiries(
    ...
    query = _with_inquiry_relations(db.query(Inquiry)).filter(Inquiry.user_id == current_user.id).order_by(
        Inquiry.created_at.desc()
    )
    items, links, meta = paginate(query, request, page, per_page=8)
    return success_response(
        {"inquiries": [_serialize_inquiry(i) for i in items], "links": links, "meta": meta}
    )
