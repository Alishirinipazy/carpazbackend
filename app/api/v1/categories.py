...
def _serialize(category: Category, with_children=False, with_parent=False, with_products=False) -> dict:
    ...
    if with_products:
        # Local import avoids a circular import with app/api/v1/cars.py
        from app.api.v1.cars import _serialize_car

        data["cars"] = [_serialize_car(c) for c in category.cars]
    return data
...
@admin_router.get("/categories/{category_id}/cars")
def admin_cars(category_id: int, db: Session = Depends(get_db), _: User = Depends(get_current_admin)):
    category = (
        db.query(Category)
        .options(joinedload(Category.cars))
