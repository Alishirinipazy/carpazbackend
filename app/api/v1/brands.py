...
def admin_index(db: Session = Depends(get_db), _: User = Depends(get_current_admin)):
    """
    Full (unpaginated) list, since brands are a short reference table -
    both for the admin brands page and for populating a brand <select> when
    creating/editing a car.
    """
    from sqlalchemy import func
    from app.models import Car

    rows = (
        db.query(Brand, func.count(Car.id).label("car_count"))
        .outerjoin(Car, Car.brand_id == Brand.id)
        .group_by(Brand.id)
        .order_by(Brand.name)
        .all()
    )
    return success_response([{**_serialize(b), "car_count": count} for b, count in rows])
...
def admin_show(brand_id: int, db: Session = Depends(get_db), _: User = Depends(get_current_admin)):
    ...
    car_count = db.query(Car).filter(Car.brand_id == brand_id).count()
    return success_response({**_serialize(brand), "car_count": car_count})
